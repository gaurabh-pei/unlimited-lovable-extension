// ============================================
// Maxx\'s Lovable – Business Logic (content)
// HTML templates are in content-templates.js
// ============================================

// integrity check removed (open-source build)

console.log("[ContentScript] Maxx\'s Lovable loaded");
...
const DISCORD_URL = (typeof DISCORD_SUPPORT_URL !== "undefined" && DISCORD_SUPPORT_URL)
  || "https://whatsapp.com/channel/0029Vb7jGDG5a245baPA9H3y";
...
function syncPkCreditBypassFromStorage() {
  ...
  if ((typeof OPEN_SOURCE_BUILD !== "undefined" && OPEN_SOURCE_BUILD) || INTERNAL_LICENSE_MODE) {
...
/** Open-source mode: create a local session so the UI opens immediately. */
...
function getBrowserSessionId() {
  return new Promise(function(resolve) {
    try {
      var fromPage = typeof pkPageStorageGet === "function" ? pkPageStorageGet("browser_session_id") : localStorage.getItem("mxx_browser_session_id");
...
function getNativeChatCaptureBody() {
  try {
    var raw = typeof pkPageStorageGet === "function" ? pkPageStorageGet("last_native_chat_capture") : localStorage.getItem("mxx_last_native_chat_capture");
    if (!raw) return null;
...
function bgFetch(url, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    if (typeof MXX_DEBUG !== "undefined" && MXX_DEBUG) console.log("[QL] bgFetch ->", url);
    chrome.runtime.sendMessage({
      action: "proxyFetch",
      url,
      ...
      if (typeof MXX_DEBUG !== "undefined" && MXX_DEBUG) console.log("[QL] bgFetch <-", url, "status", resp.status, data);
...
function showMainUI(box){
  ...
  const statusBadge = '<span class="ql-status-badge ql-badge-pro">Open Source</span>';
...
function showCustomAlert(title, message){
  try {
    if (typeof QLSounds !== "undefined" && QLSounds.errorFromMessage) {
      ...
      if (/erro|falha|negad|inv[áa]lid|expir|limite|rate|token|cr[eé]dito|sess/i.test(__ttl)) {
...
function qlRevokeAndShowLicenseGate(message) {
  ensureInternalSessionLocal().then(function() {
...
function injectShieldOverlay(){
  ...
  overlay.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' +
    ...
    '</svg>' +
    '<span class="ql-shield-overlay-text">🛡️ Protected by Maxx\'s Lovable</span>' +
...
async function uploadFileDirect(file, token) {
  ...
  // Upload through our Lovable backend. The server stores the file in our
  // private bucket and returns a long-lived signed URL we can paste into
  // the Lovable prompt. Falls back gracefully if mxxUploadAsset is missing.
...
function setupSend(){
  ...
  btn.addEventListener("click", async ()=>{
    ...
    // Images uploaded to our backend — attach public links to prompt.
    ...
    try{
      ...
      // Telemetry: report the send to our backend so it counts in admin analytics.
      try {
        if (typeof mxxTrackEvent === "function") {
          mxxTrackEvent("send", {
            prompt_length: finalMensagem.length,
            metadata: {
      ...
      try {
        if (typeof mxxTrackEvent === "function") {
          mxxTrackEvent("send_failed", { metadata: { reason: String(err && err.message || err).slice(0, 200) } });
        }
      } catch (e) {}
    } finally {
...
function injectNativeChatOverlay() {
  ...
  if (!document.getElementById("ql-native-badge")) {
    ...
    badge.innerHTML = "⚡ <span>Maxx\'s Lovable</span>";
